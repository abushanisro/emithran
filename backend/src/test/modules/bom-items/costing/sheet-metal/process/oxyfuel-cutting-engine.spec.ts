import { computeOxyfuelCost, type OxyfuelInput } from '../../../../../../modules/bom-items/costing/sheet-metal/process/oxyfuel-cutting-engine';
import { OXYFUEL_SETUP_MIN } from '../../../../../../modules/bom-items/costing/shared/core/default-rates.constants';

// Real Steel 6mm row from sm_reference_data's 'nestingCutRate:1:OxyFuelCut:Steel:200:6'
// (memory/sheetmetal/lookuptable/sheet_metal_nesting_cut_rate_combined.json,
// staged live by migration 492): feedRateLargeFeaturesMmPerMin: 603, pierceTimeS: 13.
function baseInput(overrides: Partial<OxyfuelInput> = {}): OxyfuelInput {
  return {
    sheetThicknessMm: 6,
    cutLengthMm: 1000,
    pierceCount: 2,
    batchSize: 10,
    feedRateLargeFeaturesMmPerMin: 603,
    pierceTimeSec: 13,
    ...overrides,
  };
}

describe('computeOxyfuelCost — cutting/pierce time', () => {
  it('divides cut length by the real feed rate and adds pierceCount * real pierce time', () => {
    const input = baseInput();
    const result = computeOxyfuelCost(input);
  ...
  it('stays honestly $0/0-min (no guess) when no real feed-rate/pierce-time data was resolved', () => {
    ...
    expect(result.warnings.some((w) => w.includes("nestingCutRate:*:OxyFuelCut:*"))).toBe(true);
  });
...
describe('computeOxyfuelCost — setup time', () => {
  it('uses the real resolved setup time with no warning when provided', () => {
    const input = baseInput({ setupMin: 5 });
    const result = computeOxyfuelCost(input);

    expect(result.warnings.some((w) => w.includes('setup time from fallback'))).toBe(false);
  });

  it('falls back to OXYFUEL_SETUP_MIN and discloses a warning when no real setup time is resolved', () => {
...
describe('computeOxyfuelCost — direct labour cost', () => {
  it('charges labour cost for both setup and run time when a differentiated labour rate is resolved', () => {
    const input = baseInput({
      oxyfuelRate: { rate: 80, source: 'mhr_database', machineClass: 'oxyfuel_cut', machineName: 'Test OxyFuel', commodityCode: null, labourRate: 60 },
      setupMin: 30,
    });
    const result = computeOxyfuelCost(input);
    const line = result.processLines[0]!;

    const machineOnlyRunCost = (result.cuttingMin / 60) * 80;
  ...
  it('charges $0 labour (not a guess) when no differentiated rate was resolved', () => {
    const input = baseInput({
      oxyfuelRate: { rate: 80, source: 'mhr_database', machineClass: 'oxyfuel_cut', machineName: 'Test OxyFuel', commodityCode: null, labourRate: null },
    });
    const result = computeOxyfuelCost(input);
    const line = result.processLines[0]!;
...
describe('computeOxyfuelCost — no abrasive consumable', () => {
  it('always reports abrasiveCost as 0 (oxyfuel has no garnet-equivalent consumable)', () => {
    const result = computeOxyfuelCost(baseInput());
    expect(result.abrasiveCost).toBe(0);
  });
});

describe('computeOxyfuelCost — fallback rate', () => {
  it('uses noRateFallback when no oxyfuelRate is provided, producing a defined process line rather than throwing', () => {
    const result = computeOxyfuelCost(baseInput());
    expect(result.processLines).toHaveLength(1);
    expect(result.processLines[0]!.process).toBe('OxyFuel Cut');
  });
});
